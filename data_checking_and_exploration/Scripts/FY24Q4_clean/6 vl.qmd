...
pvls_trends_country %>% 
  ...
  ggplot2::ggplot(aes(x = indicator, y=sum.results, fill=indicator)) + 
  geom_col() + 
  geom_text(aes(label = sum.results), position = position_nudge(y=10000)) + facet_grid(disagg~fyq, scales = "free_y") +  
  scale_fill_manual(values = c(TX_CURR = glitr::grey10k, TX_PVLS_D = "#64b4a6", TX_PVLS_N = "#004e43")) + 
  glitr::si_style_nolines() + theme(axis.text.x = element_text(angle=90), axis.title.x = element_blank(), legend.position = "none") + 
  labs(
...
southern_africa <- c("Zimbabwe", "Botswana", "South Africa")
