...
Notes: VLC data issues, compounding some data issues in 1e (table). Resolved 12.3
...
oneA_by_kp %>%  ggplot2::ggplot(aes(x = keypop, y = sum.cum, fill=keypop)) + 
  geom_col() + 
  # facet_grid(~ keypop) + 
  geom_text(color = glitr::grey90k, position = position_nudge(y=4000), aes(label = scales::percent(ach, accuracy = 1))) +
  geom_text(color = glitr::grey10k, position = position_stack(vjust = 0.6), aes(label = sum.cum)) + 
  glitr::si_style_nolines() +   theme(legend.position = "none") + xlab("") +
...
measure_trends_country %>% ggplot2::ggplot(aes(x = fyq, y = linkage, group=partner)) +
  ...
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 90)) + glitr::si_style_nolines() + labs(title=glue::glue("{measure_trends_country$funding_agency} {measure_trends_country$disagg} linkage by partner"))
  

measure_trends_country %>% ggplot2::ggplot(aes(x = fyq, group=partner)) +
  geom_col(aes(y = HTS_TST_POS), fill = glitr::usaid_blue) +
  geom_text(aes(y=HTS_TST_POS*1.2, label=HTS_TST_POS), color=glitr::denim) +
  geom_col(aes(y = TX_NEW), fill = glitr::grey50k, width = 0.5) + 
  geom_text(aes(y=TX_NEW*0.8, label=TX_NEW), color="black") +
  facet_wrap(~partner) + glitr::si_style_nolines() + labs(title=glue::glue("{measure_trends_country$funding_agency} {measure_trends_country$disagg} TX_NEW and HTS_TST_POS by partner"))
```

1e - by agency - CLEAN – VLC not working though –\> RECODE EXCLUDE EXCEPT vlc TO include "Lag_2"
...
measure_trends_country2 %>% ggplot2::ggplot(aes(x = fyq, y = linkage, group=keypop)) +
  ...
  # ylim(0,1.5) +
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 90)) + glitr::si_style_nolines() + 
...
measure_by_country %>% 
  ...
  geom_text(aes(label = scales::percent(linkage, accuracy = 1)), position = position_nudge(x=0.06)) + glitr::si_style_xyline()  + 
...
measure_by_country %>% 
  ...
  geom_text(aes(label = scales::percent(vls, accuracy = 1)), position = position_nudge(x=0.06)) + glitr::si_style_xyline()  + 
  # scale_x_continuous(labels = percent) +
