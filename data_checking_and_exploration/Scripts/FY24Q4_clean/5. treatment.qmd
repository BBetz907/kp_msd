...
linkage2 %>%  ggplot(aes(x = fyq, y = results, group=indicator, color=indicator)) + geom_line() + geom_text(position = position_nudge(y=30), aes(label = results)) +
  scale_color_manual(values = c(HTS_TST_POS = "#808080", TX_NEW = "#1e87a5")) + glitr::si_style_nolines() + labs(title = glue("USAID {linkage2$keypop} trends"))
...
corrected to only show quarterly data 12/31/2024
