...
prep_trends_smallerx <- prep_trends |> filter(indicator == ind[1],
  ...
  facet_wrap(~population, nrow = 1, labeller = label_wrap_gen(width = 20)) +
    scale_y_continuous(labels = comma, 
                     n.breaks = 5,
                     minor_breaks = waiver(), limits = c(0, 5000), 
                     expand = c(0, 0)) +
  si_style_ygrid() +
  theme(
      #format axes
      ...
      #format plot title
        ...
        plot.caption = element_markdown(size=11, margin = margin(t=20))) +
  labs(
    subtitle  = "USAID Global PrEP_NEW result trends and percent of total by population, excluding KP results for Uganda where KP disaggregate reporting is paused",
    ) 

prep_trends_smallerx

ggsave(plot=prep_trends_smallerx, filename = paste0("../../Images/OHA Q3/prep_trends_smallerx.png"), width = 16, height = 5)
```
...
write_csv(prep_trends_meriting_exploration, "../../Dataout/OHA Q3/prep_trends.csv")
