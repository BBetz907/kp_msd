...
grabr::pano_download(item_url = url_psnu_im, session = sess)
 ...
 dir_mer_calcs_path <- mer_items %>%
   filter(str_detect(item, "^MER Calculations Across Time")) %>%
   pull(path) |> print()
 
 mer_calc_items <- grabr::pano_content(page_url = dir_mer_calcs_path, session = sess) %>%
   grabr::pano_elements(page_url = dir_mer_calcs_path) |> print(n=29)
 # Extract MER data items details from HTML CODE
 
 # pull latest psnuXim MSD ---------------------------------------------------------
 url_mer_calc <- mer_calc_items %>%
   filter(type == "file zip_file",
          # str_detect(item, ".*_PSNU_IM_FY2.*.zip$")
   ) %>%
   pull(path) %>%
 ...
 map(url_mer_calc, ~grabr::pano_download(item_url = .x, session = sess, dest = dest_path_site))
...
dest_path <- paste0(si_path(),"/site-level calculations/")
...
short_site_file_names <- map(full_site_files, ~ str_replace(.x, "MER_Structured_Datasets_SITE_IM_SHIFT_FY21-24_20230915_Site_IM_Shift_(.*\\.zip)", "\\1")) |> print()
