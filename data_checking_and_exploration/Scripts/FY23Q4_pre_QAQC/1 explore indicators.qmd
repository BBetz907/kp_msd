...
oneA_by_kp <- check %>% filter(
  ...
  group_by(keypop, indicator, fy, standardizeddisaggregate) %>%
  summarise(sum.cum=sum(cumulative), sum.targets=sum(targets), .groups = "drop") %>%
  mutate(ach = sum.cum/sum.targets) %>% glimpse() %>%
  filter(sum.cum!=0, 
         indicator=="KP_PREV" & standardizeddisaggregate == "KeyPop") 
  
oneA_by_kp %>%  ggplot2::ggplot(aes(x = " ", y = sum.cum, fill=keypop)) + geom_col() + 
  facet_grid(~ keypop) + 
  geom_text(color = grey90k, position = position_nudge(y=4000), aes(label = percent(ach, accuracy = 1))) +
  ...
  scale_fill_manual(values = c(FSW = "#005e7a", MSM = "#f28965", TG = "#7ecfc0", Prisoners = "#990d2e", PWID = "#fbcc50")) +
  labs(title = 
         glue::glue("{oneA_by_kp$indicator} achievement by KP, {oneA_by_kp$fy}"))

oneA_by_kp %>% group_by(keypop) %>% 
  summarise(sum.cum=sum(sum.cum), sum.targets=sum(sum.targets), .groups = "drop") 

```

### 1c - checks out for USAID, USAID Ethiopia.

PEPFAR number in aggregate remain a little off likely due to known issues
...
### 1A - HTS_TST_POS kp numbers do not perfectly match USAID numbers

Country-specific numbers are fine. fixed a filter that was excluding values
...
trends_by_q <- qcheck %>%
  filter(disagg=="KP",
         str_detect(indicator, "PrEP"),
         # country %in% c("Uganda", "Tanzania"),
         fyq %in% c("FY22 Q4", "FY23 Q1", "FY23 Q2", "FY23 Q3")
         ) %>%
...
measure_trends_country2 %>% ggplot2::ggplot(aes(x = fyq, y = linkage, group=keypop)) +
  ...
  geom_text(aes(label = percent(linkage, accuracy = 1))) + 
  # ylim(0,1.5) +
  theme(legend.position = "bottom",
