...
linkage1 %>%  
  ...
  si_style_nolines() + 
  labs(title = glue::glue("PEPFAR {linkage1$disagg} trends"))
...
linkage2 %>%  ggplot(aes(x = fyq, y = results, group=indicator, color=indicator)) + geom_line() + geom_text(position = position_nudge(y=30), aes(label = results)) +
  scale_color_manual(values = c(HTS_TST_POS = "#808080", TX_NEW = "#1e87a5")) + si_style_nolines() + labs(title = glue("PEPFAR {linkage2$keypop} trends"))
