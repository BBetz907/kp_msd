...
```{r}
oneA_by_kp <- check %>% filter(country == "Kenya", 
                 disagg == "KP", 
                 fy == 2023
                 ) %>%
  group_by(keypop, indicator, fy, country) %>%
  summarise(sum.cum=sum(cumulative), sum.targets=sum(targets), .groups = "drop") %>%
  mutate(ach = sum.cum/sum.targets) %>% glimpse() %>%
  filter(sum.cum!=0, indicator=="KP_PREV") 
...
oneA_by_kp %>%  ggplot2::ggplot(aes(x = " ", y = sum.cum, fill=keypop)) + geom_col() + 
  ...
  labs(title = 
         glue::glue("{oneA_by_kp$indicator} achievement by KP in {oneA_by_kp$country}, {oneA_by_kp$fy}"))

oneA_by_kp %>% group_by(country) %>% 
...
[1b](https://tableau.usaid.gov/views/KPSlides/1_bIndicatorTrends/e86a457c-b0ee-4efb-9ac3-48376c477a2e/f0cff602-a819-485e-9b6d-0efff8f11bfe)
...
trends_by_a <- qcheck %>%
  ...
  group_by(fyq, country, disagg, indicator, keypop) %>%
  summarise(results=sum(results),.groups = "drop") 
