...
measure_trends_bots %>% ggplot2::ggplot(aes(x = psnu, y = vls)) + geom_point(size = 4) + si_style() # expanding axis, 
measure_trends_bots %>% ggplot2::ggplot(aes(x = vlc, y = vls, shape = psnu)) + geom_point(size = 4) + si_style() #label for each psnu instead, highlight outlying district
```
...
linkage_zim %>% ggplot2::ggplot(aes(x = HTS_TST_POS, y = TX_NEW, color = keypop)) + geom_point(size = 4) + si_style() #each keypop has a specific color, turn the higher ones to be a different color
