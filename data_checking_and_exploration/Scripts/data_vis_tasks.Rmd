...
indicator_list <- c("KP_PREV",  "HTS_SELF", "HTS_TST", "HTS_TST_NEG", "PrEP_NEW", "PrEP_CT", "PrEP_CURR", "HTS_TST_POS",
 ...
 df <- msd %>% filter(fiscal_year >= 2021, #cumulative and targets
                    fiscal_year < 2023,
...
#include psnu, geom_scatter
#filter by each fy, use the last quarter in each
#use check instead of qcheck
#fy22 q2

measure_trends_bots %>% ggplot2::ggplot(aes(x = TX_PVLS_D, y = TX_PVLS_N, color = psnu)) + geom_point(size = 4) + si_style()
measure_trends_bots %>% ggplot2::ggplot(aes(x = TX_CURR_Lag2, y = TX_PVLS_D, color = psnu)) + geom_point(size = 4) + si_style()

measure_trends_bots %>% ggplot2::ggplot(aes(x = psnu, y = vlc, color = psnu)) + geom_point(size = 4) + si_style()
measure_trends_bots %>% ggplot2::ggplot(aes(x = psnu, y = vls, color = psnu)) + geom_point(size = 4) + si_style()
...
```{r}
measure_trends_bots %>% ggplot2::ggplot(aes(x = HTS_TST_POS, y = TX_NEW, color = psnu)) + geom_point(size = 4) + si_style()
measure_trends_bots %>% ggplot2::ggplot(aes(x = psnu, y = linkage, color = psnu)) + geom_point(size = 4) + si_style()
```
...
measure_trends_bots %>% ggplot2::ggplot(aes(x = psnu, y = vlc)) + geom_point(size = 4) + si_style()
measure_trends_bots %>% ggplot2::ggplot(aes(x = psnu, y = vls)) + geom_point(size = 4) + si_style()
...
linkage_zim %>% ggplot2::ggplot(aes(x = HTS_TST_POS, y = TX_NEW, color = keypop)) + geom_point(size = 4) + si_style()
...
```{r}
library(ggforce)
linkage_comet <- check %>% filter(country == "India", disagg == "KP", cumulative!=0, indicator %in% measure_indicators, fy == 2022) %>%
  group_by(fy, country, indicator, keypop) %>%
  summarise(sum.cumulative=sum(cumulative)) %>%
  pivot_wider(values_from = sum.cumulative, names_from = indicator) %>%
  mutate(vls = TX_PVLS_N/TX_PVLS_D,
...
linkage_comet %>% ggplot() + geom_link(aes(x = HTS_TST_POS, y = keypop, xend = TX_NEW, yend = keypop, size = stat(index))) +
  scale_color_manual(values = c("#00A087FF", "#E64B35FF")) +
  geom_point(
    aes(x = TX_NEW, y = keypop),
    shape = 21,
    fill = "white",
    size = 3.5
  )  +
