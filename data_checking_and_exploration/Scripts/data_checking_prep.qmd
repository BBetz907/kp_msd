...
Errors running this one are due to the funding agency that is specified, so I changed it USAID for both the prevention and positive cascade.

```{r warning=FALSE}

#Thailand: Prevention Cascade
cascade_thai_prev_1 <- check %>% filter(fy == 2022, disagg == "KP", country == "Thailand",  funding_agency == "USAID", indicator %in% neg) %>%
group_by(indicator) %>%
summarise(sum.cum=sum(cumulative), sum.targets=sum(targets), .groups = 'drop') %>%
  mutate(ach = sum.cum/sum.targets) %>%
  ggplot2::ggplot(aes(x = indicator, y = sum.cum))

#error running this one
...
cascade_mal_prev_3 + geom_col(aes(y = sum.targets, color = "Targets", fill = "Targets")) + geom_col(aes(y=sum.cum, color = "Cumulative", fill = "Cumulative"), width = 0.75) + geom_label(position = position_stack(vjust = 0.9), aes(label = percent(ach, accuracy = 1, color = "Achievement", fill="Achievement")), fill = "#ee636e") + si_style_xyline() + scale_color_manual(name = "", values = c("Targets" = grey10k, "Cumulative" = denim, "Achievement" = "#ee636e")) + scale_fill_manual(name = "", values = c("Targets" = grey10k, "Cumulative" = denim, "Achievement" = "#ee636e")) + facet_wrap(~mech_code) + labs(title = "Malawi Prevention Cascade, IM = 81764 and 81759")
...
this is great. no color legend needed though.
...
measure_trends_ug_kp_1c_hts %>% ggplot2::ggplot(aes(x = fyq, y = sum.results)) + geom_line(aes(color = sum.results), group = 1) + si_style_xyline() + facet_wrap(~keypop) + geom_text(position = position_stack(), aes(label = sum.results)) + labs(title = "Uganda: HTS_TST by KP") 

measure_trends_ug_kp_1c_hts_pos <- qcheck %>% filter(country == "Uganda", disagg == "KP", results!=0, indicator== "HTS_TST_POS") %>%
  group_by(fyq, keypop) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>% na.omit()

measure_trends_ug_kp_1c_hts_pos %>% ggplot2::ggplot(aes(x = fyq, y = sum.results)) + geom_line(aes(color = sum.results), group = 1) + si_style_xyline() + facet_wrap(~keypop) + geom_text(position = position_stack(), aes(label = sum.results)) + labs(title = "Uganda: HTS_TST_POS by KP") 
...
measure_trends_ug_fa_1c_hts %>% ggplot2::ggplot(aes(x = fyq, y = sum.results)) + geom_line(aes(color = sum.results), group = 1) + si_style_xyline() + facet_wrap(~funding_agency) + geom_text(position = position_stack(), aes(label = sum.results)) + labs(title = "Uganda: HTS_TST by Funding Agency") 

measure_trends_ug_fa_1c_hts_pos <- qcheck %>% filter(country == "Uganda", disagg == "KP", results!=0, indicator== "HTS_TST_POS") %>%
  group_by(fyq, funding_agency) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>% na.omit()

measure_trends_ug_fa_1c_hts_pos %>% ggplot2::ggplot(aes(x = fyq, y = sum.results)) + geom_line(aes(color = sum.results), group = 1) + si_style_xyline() + facet_wrap(~funding_agency) + geom_text(position = position_stack(), aes(label = sum.results)) + labs(title = "Uganda: HTS_TST_POS by Funding Agency") 
```
...
measure_trends_moz_kp_1c_hts %>% ggplot2::ggplot(aes(x = fyq, y = sum.results)) + geom_line(aes(color = sum.results), group = 1) + si_style_xyline() + facet_wrap(~keypop) + geom_text(position = position_stack(), aes(label = sum.results)) + labs(title = "Mozambique: HTS_TST by KP") 

measure_trends_moz_kp_1c_hts_pos <- qcheck %>% filter(country == "Mozambique", disagg == "KP", results!=0, indicator== "HTS_TST_POS") %>%
  group_by(fyq, keypop) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>% na.omit()

measure_trends_moz_kp_1c_hts_pos %>% ggplot2::ggplot(aes(x = fyq, y = sum.results)) + geom_line(aes(color = sum.results), group = 1) + si_style_xyline() + facet_wrap(~keypop) + geom_text(position = position_stack(), aes(label = sum.results)) + labs(title = "Mozambique: HTS_TST_POS by KP") 
...
measure_trends_moz_fa_1c_hts %>% ggplot2::ggplot(aes(x = fyq, y = sum.results)) + geom_line(aes(color = sum.results), group = 1) + si_style_xyline() + facet_wrap(~funding_agency) + geom_text(position = position_stack(), aes(label = sum.results)) + labs(title = "Mozambique: HTS_TST by Funding Agency") 

measure_trends_moz_fa_1c_hts_pos <- qcheck %>% filter(country == "Mozambique", disagg == "KP", results!=0, indicator== "HTS_TST_POS") %>%
  group_by(fyq, funding_agency) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>% na.omit()

measure_trends_moz_fa_1c_hts_pos %>% ggplot2::ggplot(aes(x = fyq, y = sum.results)) + geom_line(aes(color = sum.results), group = 1) + si_style_xyline() + facet_wrap(~funding_agency) + geom_text(position = position_stack(), aes(label = sum.results)) + labs(title = "Mozambique: HTS_TST_POS by Funding Agency") 
```
