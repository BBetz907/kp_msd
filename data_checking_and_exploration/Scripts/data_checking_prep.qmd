...
cascade_mal_pos_q <- qcheck %>% filter(fy == 2022, disagg == "KP", country == "Malawi", mech_code %in% mal_im, indicator %in% pos) %>%
group_by(fyq, indicator) %>% 
summarise(sum.results=sum(results), .groups = 'drop') 
...
#### Cameroon: PrEP_CT, HTS_TST_POS - By KP, funding Agency
...
#By KP
measure_trends_cam_kp_prep <- check %>% filter(fy == 2022, disagg == "KP", country == "Cameroon", indicator =="PrEP_CT") %>%
...
measure_trends_cam_kp_hts <- check %>% filter(fy == 2022, disagg == "KP", country == "Cameroon", indicator =="HTS_TST_POS") %>%
  group_by(keypop, indicator) %>%
  summarise(sum.cumulative=sum(cumulative), .groups = "drop")
...
#By Funding Agency
measure_trends_cam_fa_prep <- check %>% filter(fy == 2022, disagg == "KP", country == "Cameroon", indicator== "PrEP_CT") %>%
...
measure_trends_cam_fa_hts <- check %>% filter(fy == 2022, disagg == "KP", country == "Cameroon", indicator== "HTS_TST_POS") %>%
  group_by(funding_agency) %>%
  summarise(sum.cumulative=sum(cumulative), .groups = "drop")
...
#### Uganda: PrEP_CT, HTS_TST_POS - By KP, funding Agency

```{r warning=FALSE}
#Uganda

#By KP
measure_trends_ug_kp_prep <- check %>% filter(fy == 2022, disagg == "KP", country == "Uganda", indicator =="PrEP_CT") %>%
  group_by(keypop, indicator) %>%
  summarise(sum.cumulative=sum(cumulative), .groups = "drop")
...
measure_trends_ug_kp_hts <- check %>% filter(fy == 2022, disagg == "KP", country == "Uganda", indicator =="HTS_TST_POS") %>%
  group_by(keypop, indicator) %>%
  summarise(sum.cumulative=sum(cumulative), .groups = "drop")
...
#By Funding Agency
measure_trends_ug_fa_prep <- check %>% filter(fy == 2022, disagg == "KP", country == "Uganda", indicator== "PrEP_CT") %>%
  group_by(funding_agency) %>%
  summarise(sum.cumulative=sum(cumulative), .groups = "drop")
...
measure_trends_ug_fa_hts <- check %>% filter(fy == 2022, disagg == "KP", country == "Uganda", indicator== "HTS_TST_POS") %>%
  group_by(funding_agency) %>%
  summarise(sum.cumulative=sum(cumulative), .groups = "drop")
...
#### Uganda by KP, by Agency

```{r warning=FALSE}
#Uganda

#By KP
measure_trends_ug_kp_1c_hts <- qcheck %>% filter(country == "Uganda", disagg == "KP", results!=0, indicator== "HTS_TST") %>%
  group_by(fyq, keypop) %>%
...
measure_trends_ug_kp_1c_hts_pos <- qcheck %>% filter(country == "Uganda", disagg == "KP", results!=0, indicator== "HTS_TST_POS") %>%
  group_by(fyq, keypop) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>% na.omit()
...
#By Funding Agency
measure_trends_ug_fa_1c_hts <- qcheck %>% filter(country == "Uganda", disagg == "KP", results!=0, indicator== "HTS_TST") %>%
...
measure_trends_ug_fa_1c_hts_pos <- qcheck %>% filter(country == "Uganda", disagg == "KP", results!=0, indicator== "HTS_TST_POS") %>%
  group_by(fyq, funding_agency) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>% na.omit()
...
#### Mozambique by KP, by Agency

```{r warning=FALSE}
#Mozambique

#By KP
measure_trends_moz_kp_1c_hts <- qcheck %>% filter(country == "Mozambique", disagg == "KP", results!=0, indicator== "HTS_TST") %>%
  group_by(fyq, keypop) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>% na.omit()
...
measure_trends_moz_kp_1c_hts_pos <- qcheck %>% filter(country == "Mozambique", disagg == "KP", results!=0, indicator== "HTS_TST_POS") %>%
  group_by(fyq, keypop) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>% na.omit()
...
#By Funding Agency
measure_trends_moz_fa_1c_hts <- qcheck %>% filter(country == "Mozambique", disagg == "KP", results!=0, indicator== "HTS_TST") %>%
  group_by(fyq, funding_agency) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>% na.omit()
...
measure_trends_moz_fa_1c_hts_pos <- qcheck %>% filter(country == "Mozambique", disagg == "KP", results!=0, indicator== "HTS_TST_POS") %>%
  group_by(fyq, funding_agency) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>% na.omit()
...
measure_trends_viet_2a %>% ggplot2::ggplot(aes(x = fyq, y = results, group = indicator)) + geom_line(aes(color = indicator)) + si_style_xyline() + ylim(0,1000) + geom_text(aes(label = results)) + labs(title = "Vietnam Testing and Linkage Trends") 
...
measure_trends_viet_2b <- qcheck %>% filter(country == "Philippines", disagg == "KP", results!=0, indicator %in% indicators_2b, funding_agency %in% agency_2a, partner == "Family Health International") %>%
  group_by(fyq, indicator) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>%
...
#### South Africa: MSM Only and USAID All People
...
#MSM Only
measure_trends_laos_2b_msm <- qcheck %>% filter(country == "South Africa", disagg == "KP", results!=0, indicator %in% indicators_2b, funding_agency == "USAID", keypop == "MSM") %>%
  group_by(fyq, indicator) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>%
  pivot_wider(values_from = sum.results, names_from = indicator) %>% pivot_longer(TX_NEW:TX_NET_NEW, names_to = "indicator", values_to = "results") %>% na.omit()
...
#USAID All People
measure_trends_laos_2b_all <- qcheck %>% filter(country == "South Africa", results!=0, indicator %in% indicators_2b, funding_agency == "USAID", disagg == "Total") %>%
  group_by(fyq, indicator) %>%
  summarise(sum.results=sum(results), .groups = "drop") %>%
  pivot_wider(values_from = sum.results, names_from = indicator) %>% pivot_longer(TX_NEW:TX_NET_NEW, names_to = "indicator", values_to = "results") %>% na.omit()
...
#Malawi
measure_trends_ind_2f <- qcheck %>% filter(country == "Malawi", disagg == "KP", results!=0, indicator %in% indicators_2f, keypop %in% kp_2f) %>%
