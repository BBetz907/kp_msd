...
qcheck <- df %>% filter(fiscal_year >= 2022, #cumulative and targets
  ...
  select(operatingunit, country, snu1, psnu, partner, mech_code, mech_name, indicator, funding_agency, numeratordenom, disagg, standardizeddisaggregate, otherdisaggregate, keypop, kp_prev_status, fy, qtr1, qtr2, qtr3, qtr4) %>%
  mutate(indicator = factor(indicator, levels = indicator_list)) %>% arrange(indicator) %>%
