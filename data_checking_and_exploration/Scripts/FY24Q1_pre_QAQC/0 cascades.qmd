...
PEPFAR 2024 all correct. USAID and DOD correct for prior years, small discrepancies for CDC.
...
compare_cascade_by_b <- check %>%
  filter(country=="Nepal",                            
         ...
         disagg == "KP",) %>%
  group_by(indicator, keypop, fy, country, funding_agency) %>%
  summarise(sum.cum = sum(cumulative),
            sum.targ = sum(targets),
            ach = sum(cumulative)/sum(targets),
            .groups = "drop")
...
compare_cascade_by_b %>% ggplot(aes(x=indicator)) +
  ...
  theme(panel.background = element_blank(),
        legend.position = "none",
        axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = glue::glue("{compare_cascade_by_b$funding_agency}  {compare_cascade_by_b$country} prevention cascade by key pop, {compare_cascade_by_b$fy}"),y="cumulative")
```
